import { DashBoardQRProjectOverridesCellIndexes, DashboardsConfigs, EnuKPI, FindTableIndexesForScriptConditionsResult, MandatoryEpicDesignationFieldsCellIndexes, SearchConditionsCellIndexes, SearchScripts } from "./_models";
...
export class GetDashBoardAndConditions {
  ...
  private scriptConditionStartParser(indexes: FindTableIndexesForScriptConditionsResult, sheet: GoogleSpreadsheetWorksheet) {
    ...
    for (let rowIndex = indexes.indexForSearchConditionsStart; rowIndex <= indexes.indexForSearchConditionsEnd; rowIndex++) {
      ...
      if (script_name) {
        GetDashBoardAndConditions.dashboardsConfigs.searchConditions[script_name] = {
