...
export class GetPrintedIssuesList {
  ...
  public async updateIssueWorksheetHeader(activeWeek: number): Promise<GoogleSpreadsheetWorksheet> {
    ...
    const spreadSheet = await this.configApp.googleServices.spreadsheet.getSpreadsheetWithWorksheet(
  ...
  public async getIssueWorksheet(activeWeek: number): Promise<ActiveIssueWorkSheetWithIssues> {
    console.log(
      "ActiveIssueWorkSheetWithIssues:getIssueWorksheet: init =========================================================================",
    );
    ...
    if (GetPrintedIssuesList.weekSheetCopy[activeWeek + ""]) {
      ...
      for (const row of rows) {
        issueWorkSheet.issueList.push({
          ...
          responsibilityOwnerName:
            result.sheet.getCellByA1(issueWorkSheet.cells.cells.responsibilityOwnerCell + row.rowNumber).stringValue ?? "",
          ...
          responsibilityOwnerEmail: result.sheet.getCellByA1(issueWorkSheet.cells.cells.responsibilityOwnerEmailCell + row.rowNumber)
            .stringValue,
