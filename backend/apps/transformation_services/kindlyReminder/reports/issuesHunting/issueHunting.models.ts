import { SearchCondition } from "../dashboard/dashboardKR.models";
...
export class JQLProjectQueriesAutomation {
  public findNonEpicsWithEpicDesignation: string =
    "type != Epic" + ' AND "epic designation[dropdown]" IN (Feature, KTLO)' + " ORDER BY created DESC";

  public findKTLOEpicsWithoutEpicDesignation: string =
    "type = Epic" +
  ...
  public findAndFixAllNonHardlySetEpicDesignations: string =
    "project IN ({projects})" +
    " AND type = Epic" +
...
export class JQLProjectQueries {
  ...
  // Parent - epic
  public findAllEpicTicketsWithoutParent: string =
    'project = "{projectKey}"' +
    " AND parent IS empty" +
  ...
  // Parent - initiative
  public findAllInitiativesTicketsWithoutParent: string =
    'project = "{projectKey}"' +
    " AND parent IS empty" +
    ' AND created >= "2024-04-01"' +
  ...
  // Owner - epic
  public findAllEpicTicketsWithoutOwner: string =
    'project = "{projectKey}"' +
  ...
  // Owner - initiative
  public findAllInitiativeTicketsWithoutOwner: string =
    'project = "{projectKey}"' +
    " AND assignee IS EMPTY" +
    " AND type IN (Initiative)" +
    ' AND created >= "2024-04-01"' +
  ...
  // Owner - theme
  public findAllThemesTicketsWithoutOwner: string =
    'project = "{projectKey}"' + " AND assignee IS EMPTY" + " AND type IN (Themes)" + " ORDER BY created DESC";

  // Owner - all tickets in sprint
  public findSprintTicketsWithoutOwner: string =
    'project = "{projectKey}"' +
  ...
  public findAllInitiativeWithoutProperField: string =
    'project = "{projectKey}"' +
    " AND assignee IS EMPTY" +
    " AND type = Initiative" +
    ' AND status IN ("Parking lot", "To Do", "In Progress", Blocked)' +
  ...
  // Labels required - initiative - (initiative category)
  public findAllInitiativeWithoutFinancialDescription =
    'project = "{projectKey}"' +
    " AND type = Initiative" +
  ...
  // Labels required - initiative - (initiative category)
  public findAllInitiativeWithoutCustomerFacing: string =
    'project = "{projectKey}"' +
    " AND type = Initiative" +
  ...
  public findAllThemesWithoutProperField: string =
    'project = "{projectKey}"' + " AND type = Themes" + ' AND "strategic objectives[dropdown]" is EMPTY' + " ORDER BY created DESC";

  // Labels required - Themes - (Epic Designation)
  public findAllEpicsWithoutProperFieldEpicDesignation: string =
    'project = "{projectKey}"' +
    " AND type = Epic" +
  ...
  public findSprintTicketsWithoutOriginalEstimate: string =
    'project = "{projectKey}"' +
