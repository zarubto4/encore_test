import { UserStage } from "./prepareUserStatsStage.service";
...
export class SynchronizeUserWorkSheet {
  ...
  public async synchronize(): Promise<void> {
    ...
    log.trace(
      "SynchronizeUserWorkSheet:getActiveUserWorkSheet: total users in worksheet: " + Object.keys(result.userWorkSheet.users).length,
    );
    ...
    for (const user of users) {
      ...
      if (result.userWorkSheet.users[user.emailAddress]) {
        ...
        result.sheet.getCellByA1(
          result.userWorkSheet.cells.userCells.userEmailColum + result.userWorkSheet.users[user.emailAddress].row,
