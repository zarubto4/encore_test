...
export class TempoService {
  ...
  public getWorkLogsByIssue(issueId: string, request: Tempo_wokLogsRequest, pageForPagination = 0): Promise<WorkLog[]> {
    return new Promise((resolve): void => {
      ...
      this.request.get<Tempo_workLogsResponse>("/worklogs", request, 200).then((result) => {
        ...
        if (result.error) {
          ...
          if ((result.data && result.data.metadata.next) != null) {
            this.getWorkLogsByIssue(issueId, request, pageForPagination + this.pagingSize).then((subResult) => {
              try {
                // Merge the results and return them
                resolve([...new Set(result.data?.results.concat(subResult))]);
              } catch (error) {
  ...
  public getWorkLogsByIssueList(
    issueIds: string[],
    request: Tempo_wokLogsRequest,
    pageForPagination = 0,
  ): Promise<{ workLogs: WorkLog[]; request: Tempo_wokLogsRequest }> {
    return new Promise((resolve): void => {
      ...
      this.request.post<Tempo_workLogsResponse>("/worklogs/search", query, requestSearch, 200).then((result) => {
        if (result.error) {
          ...
          if ((result.data && result.data.metadata.next) != null) {
            ...
            this.getWorkLogsByIssueList(issueIds, request, pageForPagination + this.pagingSize).then((subResult) => {
              try {
                resolve({
                  request: request,
      ...
      this.request.get<Tempo_workLogsResponse>("/worklogs", request, 200).then((result) => {
        if (result.error) {
          ...
          if (result.data && result.data.metadata.next != null) {
            ...
            this.getUsersWorkLogs(request, pageForPagination + this.pagingSize).then((subResult) => {
              // Merge the results and return them
