...
import {Projects, ProjectWorkSheet, WeekProjectWorkSheetCell} from "./_models";
import {
...
export class FindAndSynchronizeProjects {
    ...
    public async  prepareProjects(): Promise<void> {
        console.log("ProjectStage:prepareProjects: init =========================================================================");
            ...
            for (let projectKey of Object.keys(projects)) {
                ...
                if (!projects[project.key]) {
                    ...
                    if (project.lead && project.lead.displayName != "Former user" && project.lead.active) {
                        ...
                        if(!project.lead.emailAddress) {
                            console.log("User - missing email - load separatly");
                            await this.configApp.jiraServices.user.getUser({accountId: project.lead.accountId})
                                .then((user) => {
                                    if(user) {
                    ...
                    if (managerWorksheet && project.lead.displayName && managerWorksheet.managerStructure[project.lead.displayName]) {
                        ...
                        if (managerNameCell.stringValue == "No VP" || !managerNameCell.stringValue) {
                            if (managerWorksheet.managerStructure[project.lead.displayName].vicePresidentLevelManager ) {
                                ...
                                const mngr = managerWorksheet.managerStructure[project.lead.displayName].vicePresidentLevelManager;
                                if( mngr && kindlyReminder_hideThiesVps.includes( mngr.workerName)) {
                                    switch (mngr.workerName) {
                                        case "Barbara Weisz": {
                                            managerNameCell.value = "non-PE (SLS)";
                                            break;
                                        }
                                        case "Jiri Ponrt": {
                                            managerNameCell.value = "non-PE (FIN)";
                                            break;
                                        }
                                        case "Adam Lindsey": {
                                            managerNameCell.value = "non-PE (GO)";
                                            break;
                                        }
                                        case "Zdenek Linc": {
                                            managerNameCell.value = "non-PE (MAR)";
                                            break;
                                        }
                                    }

                                } else if (mngr) {
                                    managerNameCell.value = mngr.workerName;
                                    managerEmailCell.value = mngr.workerEmail;
                                }
                        ...
                        // Synchronise VP emails
                            ...
                            if (userFinder) {
                                console.log("Manager - missing email - load separatly: What we found:", JSON.stringify(userFinder, null, 2));
                                await this.configApp.jiraServices.user.getUser({accountId: userFinder.accountId})
                                    .then((user) => {
    ...
    public fillZeroInNonIssuedProjects(config: ProjectWorkSheet): Promise<void>  {
